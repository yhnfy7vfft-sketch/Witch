...
- (void)refreshBuffer {
    if (!runtimeJavaVMPtr) {
        ...
        // Obtain JNIEnvs
        (*runtimeJavaVMPtr)->AttachCurrentThreadAsDaemon(runtimeJavaVMPtr, &surfaceJNIEnv, NULL);
        if (!surfaceJNIEnv) return;
        dispatch_async(dispatch_get_main_queue(), ^{
            (*runtimeJavaVMPtr)->AttachCurrentThreadAsDaemon(runtimeJavaVMPtr, &runtimeJNIEnvPtr, NULL);
