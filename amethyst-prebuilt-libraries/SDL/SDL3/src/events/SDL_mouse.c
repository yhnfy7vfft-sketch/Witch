...
bool SDL_MousePositionInWindow(SDL_Window *window, float x, float y)
{
    ...
    if (window && !(window->flags & SDL_WINDOW_MOUSE_CAPTURE)) {
        /* Local fix: check against the window size in pixels, not the logical
         * size. On iOS (UIKit) the logical size is in points while injected
         * mouse events carry framebuffer-pixel coordinates; comparing against
         * points would flag every pixel-space event outside the window and
         * drop the mouse focus. */
        int w, h;
...
/* Local: the host launcher always knows the real cursor position (it owns
 * the BT mouse). Warps performed by SDL (e.g. the grab-release warp in
 * SDL_SetRelativeMouseMode) clear has_position, making SDL_GetMouseState
 * report (0,0) until the next motion. Sync the internal state to the
 * launcher's clamped coordinates without posting any event, so games that
 * read the cursor position from the state (menu look direction, GUI cursor)
 * always see where the cursor actually is.
 *
 * IMPORTANT: do NOT touch last_x/last_y here. SDL_PrivateSendMouseMotion
 * computes xrel/yrel against last_x/last_y and drops events whose delta is
 * zero, so clobbering them from the routing hook would swallow every
 * absolute (menu) motion and freeze the game's cursor. */
void AASDL_SyncMouseState(float x, float y)
{
    SDL_Mouse *mouse = SDL_GetMouse();
    if (mouse) {
        mouse->x = x;
...
static void ConstrainMousePosition(SDL_Mouse *mouse, SDL_Window *window, float *x, float *y)
{
    ...
    if (window && !(window->flags & SDL_WINDOW_MOUSE_CAPTURE)) {
        /* Local fix: use the window size in pixels, not the logical size.
         * Injected mouse events carry framebuffer-pixel coordinates (see
         * SDL_MousePositionInWindow), so constraining against the logical
         * (points) size would clamp every event to a fraction of the window
         * and confine the cursor to that fraction. */
        int pixel_w, pixel_h;
        SDL_GetWindowSizeInPixels(window, &pixel_w, &pixel_h);
        int x_min = 0, x_max = pixel_w - 1;
        int y_min = 0, y_max = pixel_h - 1;
