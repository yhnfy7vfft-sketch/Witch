...
/* Local fix: iOS destroys GPU command buffers submitted while the app is
 * backgrounded ("BackgroundExecutionNotPermitted"), which loses the Vulkan
 * device and crashes the game on re-entry. The game's render thread drives
 * its frame loop through SDL_PollEvent, so while the app is not active we
 * sleep here and no frames get submitted. The flag is toggled from the
 * SDL_OnApplication* lifecycle hooks (see SDL_video.c). */
static bool aasdl_sdl_backgrounded;

void SDL_SetBackgroundedFlag(bool backgrounded)
{
    aasdl_sdl_backgrounded = backgrounded;
}

bool SDL_PollEvent(SDL_Event *event)
{
    while (aasdl_sdl_backgrounded) {
        SDL_DelayNS(100 * SDL_NS_PER_MS);
    }
...
bool SDL_PushEvent(SDL_Event *event)
{
    ...
    /* Local: the host app (launcher) injects input through SDL_PushEvent.
     * Mouse events must be routed through the real mouse pipeline: SDL_PushEvent
     * alone only queues the event and leaves SDL_GetMouseState() stale, while
     * GLFW-style consumers (glfwGetCursorPos/glfwGetMouseButton through
     * lwjgl-sdl) read the internal state every frame. The mouse window is
     * derived from the focus, so no window-list lookup happens here.
     *
     * Guard: SDL_SendMouseMotion/Button/Wheel post their own event through
     * SDL_PushEvent, so without this flag every injected mouse event would
     * recurse forever. The chain is synchronous on one thread, so a plain
     * bool is sufficient; a concurrent caller simply skips the redundant
     * state sync for that event. */
    static bool sdl_push_mouse_routing;
    if (event->type == SDL_EVENT_MOUSE_MOTION) {
        if (!sdl_push_mouse_routing) {
            sdl_push_mouse_routing = true;
            /* Local fix: decide relative vs absolute from the ACTUAL mouse
             * mode, not from xrel/yrel != 0. The launcher always fills both
             * fields (x/y = accumulated position, xrel/yrel = delta), so a
             * zero-delta event during play carried an out-of-bounds absolute
             * position and was misrouted as an absolute teleport, pushing the
             * game's cursor off-screen (menu mouse appeared dead). Also pass
             * the real window so SDL_PrivateSendMouseMotion constrains the
             * position to the window bounds in relative mode. Finally, sync
             * the internal cursor state to the launcher's clamped position:
             * warps (e.g. the grab-release warp on menu open) clear
             * has_position, which makes SDL_GetMouseState report (0,0) and
             * the game look at the top-left corner. */
            bool relative = SDL_GetRelativeMouseMode();
            float x = relative ? event->motion.xrel : event->motion.x;
            float y = relative ? event->motion.yrel : event->motion.y;
            AASDL_SyncMouseState(event->motion.x, event->motion.y);
            SDL_Window *w = SDL_GetWindowFromID(event->motion.windowID);
            SDL_SendMouseMotion(event->motion.timestamp, w, SDL_GLOBAL_MOUSE_ID, relative, x, y);
            sdl_push_mouse_routing = false;
            return true;
        }
    }
    if (event->type == SDL_EVENT_MOUSE_BUTTON_DOWN || event->type == SDL_EVENT_MOUSE_BUTTON_UP) {
        if (!sdl_push_mouse_routing) {
            sdl_push_mouse_routing = true;
            SDL_SendMouseButton(event->button.timestamp, NULL, SDL_GLOBAL_MOUSE_ID,
                                event->button.button, event->type == SDL_EVENT_MOUSE_BUTTON_DOWN);
            sdl_push_mouse_routing = false;
            return true;
        }
    }
    if (event->type == SDL_EVENT_MOUSE_WHEEL) {
        if (!sdl_push_mouse_routing) {
            sdl_push_mouse_routing = true;
            SDL_SendMouseWheel(event->wheel.timestamp, NULL, SDL_GLOBAL_MOUSE_ID,
