...
/* Local (non-upstream) bridge: notifies the host app (AngelAuraAmethyst) about
 * the SDL window id/size so it can feed input and lay out its UI without
 * calling SDL_GetWindows()/SDL_GetWindowSize() from other threads (those walk
 * the window list without locking). Resolved lazily; never cached per window. */
static void (*sdl_bridge_NoteWindow)(Uint32 windowID, int w, int h);

static void UIKit_NoteWindow(SDL_Window *window, int w, int h)
{
    if (!sdl_bridge_NoteWindow) {
        sdl_bridge_NoteWindow = (void (*)(Uint32, int, int))dlsym(RTLD_DEFAULT, "AASDL_NoteWindow");
    }
    if (sdl_bridge_NoteWindow) {
        sdl_bridge_NoteWindow(window ? window->id : 0, w, h);
    }
}

/* Local (non-upstream) bridge: asks the host app for the UIView the game
 * should render into (the launcher's game surface). When non-nil, SDL skips
 * creating its own UIWindow so the game stays inside the launcher's view
 * hierarchy and follows its layout (controls, edge-swipe menu, rotation). */
static UIView *UIKit_GetHostView(void)
{
    static UIView *(*GetHostView)(void);
    if (!GetHostView) {
        GetHostView = (UIView *(*)(void))dlsym(RTLD_DEFAULT, "AASDL_GetHostView");
    }
    return GetHostView ? GetHostView() : nil;
}

static bool SetupWindowData(SDL_VideoDevice *_this, SDL_Window *window, UIWindow *uiwindow, bool created, bool hostWindow)
{
...
{
    // UIWindow creation and UIApplication state changes must run on the main
    // thread; SDL may be driving the video subsystem from a non-main thread
    // when embedded in an existing app.
        ...
        @autoreleasepool {
            ...
            for (other = _this->windows; other; other = other->next) {
                if (other != window && SDL_GetVideoDisplayForWindow(other) == display) {
                    ret = SDL_SetError("Only one window allowed per display.");
                    return;
                }
            }
            ...
            /* Local bridge: on the main screen, prefer the host app's window so
             * the game renders inside the launcher's view hierarchy. */
            UIView *hostView = UIKit_GetHostView();
            if (data.uiscreen == [UIScreen mainScreen] && hostView != nil && hostView.window != nil) {
                uiwindow = hostView.window;
                hostWindow = YES;
            ...
            if (!SetupWindowData(_this, window, uiwindow, true, hostWindow)) {
                return;
            }
    ...
    dispatch_sync(dispatch_get_main_queue(), ^{
        @autoreleasepool {
            ...
            if (!data.hostWindow) {
                ...
                data.uiwindow.hidden = NO;
                data.viewcontroller.view.hidden = NO;
            }

            /* Local bridge: tell the host app the SDL window is live. */
            UIKit_NoteWindow(window, window->w, window->h);
...
void UIKit_HideWindow(SDL_VideoDevice *_this, SDL_Window *window)
{
    dispatch_sync(dispatch_get_main_queue(), ^{
        @autoreleasepool {
            SDL_UIKitWindowData *data = (__bridge SDL_UIKitWindowData *)window->internal;
            if (data.hostWindow) {
                /* Must not hide the host app's window, only our view. */
                data.viewcontroller.view.hidden = YES;
            } else {
    ...
    dispatch_sync(dispatch_get_main_queue(), ^{
        @autoreleasepool {
            if (window->internal != NULL) {
                 ...
                 * SDL window. A host window must keep its own root view controller
                 * and visibility. */
                if (data.hostWindow) {
                    data.viewcontroller.view.hidden = NO;
                } else {
                ...
                /* Local bridge: tell the host app the SDL window is gone. */
                UIKit_NoteWindow(NULL, 0, 0);
            }
        }
    });
}
...
{
    @autoreleasepool {
        /* Local fix: the host launcher owns the render surface (CAMetalLayer
         * contentsScale = UIScreen.scale x resolutionScale) and reports that
         * framebuffer size to the game; injected mouse events carry coordinates
         * in that space. Deriving the pixel size from UIScreen.nativeScale
         * instead gives a different size on devices where nativeScale != scale
         * (e.g. iPhone 6/7/8 Plus: 2.6087 vs 3.0), so SDL would clamp mouse
         * events and drop the mouse focus before the bottom of the screen.
         * Ask the launcher for its framebuffer size first; fall back to the
         * stock bounds x nativeScale computation if unavailable. */
        static void (*AASDL_GetFramebufferSize)(int *w, int *h);
        static bool looked_up;
        if (!looked_up) {
            AASDL_GetFramebufferSize = (void (*)(int *, int *)) dlsym(RTLD_DEFAULT, "AASDL_GetFramebufferSize");
            looked_up = true;
        }
        if (AASDL_GetFramebufferSize) {
            int fw = 0, fh = 0;
            AASDL_GetFramebufferSize(&fw, &fh);
            if (fw > 0 && fh > 0) {
                *w = fw;
                *h = fh;
                return;
