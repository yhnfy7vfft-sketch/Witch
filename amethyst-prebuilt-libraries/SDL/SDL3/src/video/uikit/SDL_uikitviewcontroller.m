...
- (void)viewDidLayoutSubviews
{
    ...
    /* Local bridge: keep the host app's window size snapshot in sync. */
    static void (*noteWindow)(Uint32 windowID, int w, int h);
    if (!noteWindow) {
        noteWindow = (void (*)(Uint32, int, int))dlsym(RTLD_DEFAULT, "AASDL_NoteWindow");
    }
    if (noteWindow) {
        noteWindow(window->id, w, h);
    }
}
...
- (NSArray *)keyCommands
{
    // Local modification: when the game view is embedded in the host app's
    // surface, the host launcher handles the hardware keyboard itself (UIKey
    // events -> GLFW/SDL injection). SDL registering these commands would
    // deliver the same physical keys twice to the game.
    if ([self.view respondsToSelector:@selector(isHostWindowEmbedded)] && [(SDL_uikitview *)self.view isHostWindowEmbedded]) {
        return @[];
    }
...
- (bool)startTextInput
{
    ...
    // Local modification: when an external (Bluetooth) keyboard is attached,
    // don't pop up the on-screen virtual keyboard; typed text arrives through
    // the host launcher's UIKey events -> SDL text input injection instead.
    // Some Bluetooth keyboards are not exposed through GCKeyboard.coalescedKeyboard,
    // so also trust the launcher's report of recently pressed physical keys.
    static bool (*AASDL_HardwareKeySeenWithinFn)(double);
    if (!AASDL_HardwareKeySeenWithinFn) {
        AASDL_HardwareKeySeenWithinFn = dlsym(RTLD_DEFAULT, "AASDL_HardwareKeySeenWithin");
    }
    bool hardwareKeyActive = AASDL_HardwareKeySeenWithinFn ? AASDL_HardwareKeySeenWithinFn(120.0) : false;
    if (@available(iOS 14.0, *)) {
        if (GCKeyboard.coalescedKeyboard != nil || hardwareKeyActive) {
...
- (bool)stopTextInput
{
    ...
    // Local modification: restore the normal input view (see startTextInput).
    textField.inputView = nil;
...
bool UIKit_StartTextInput(SDL_VideoDevice *_this, SDL_Window *window, SDL_PropertiesID props)
{
    @autoreleasepool {
        ...
        if (!NSThread.isMainThread) {
            /* Local: the game (Minecraft 26.x) calls this from its render
             * thread; becomeFirstResponder must run on the main thread or
             * UIKit asserts (FBSSerialQueue assertBarrierOnQueue). */
