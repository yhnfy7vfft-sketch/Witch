...
- (void)setSDLWindow:(SDL_Window *)window
{
    ...
    if (window) {
        ...
        if (data.hostWindow) {
            /* Local: embed the game view into the host app's surface so it
             * stays inside the launcher's view hierarchy (it must follow the
             * launcher layout, e.g. the edge-swipe menu shrinking the game). */
            static UIView *(*GetHostView)(void);
            if (!GetHostView) {
                GetHostView = (UIView *(*)(void))dlsym(RTLD_DEFAULT, "AASDL_GetHostView");
            }
            UIView *hostView = GetHostView ? GetHostView() : nil;
            if (hostView) {
...
/* Local: when the game view is embedded into the host app's surface
 * (hostWindow), the host owns all touch input: the launcher translates touches
 * into GLFW/SDL mouse events itself. Without this, SDL_uikitview swallows
 * every touch and the launcher's gesture recognizers / touch handlers never
 * fire (cursor stops moving, taps never click). Forwarding to super walks the
 * touch up the responder chain to the host's view controller. */
- (BOOL)isHostWindowEmbedded
{
    if (!sdlwindow) {
        return NO;
    }
    SDL_UIKitWindowData *data = (__bridge SDL_UIKitWindowData *)sdlwindow->internal;
    return data.hostWindow;
}

- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    if ([self isHostWindowEmbedded]) {
...
/* Local modification: derive a real SDL_Keycode from the UIKey so the game
   receives usable keycodes instead of SDLK_UNKNOWN (the SDL keymap is never
   populated on this platform and SDL_SendKeyboardKey(..., 0, ...) would
   otherwise report keycode 0 for every key). */
- (SDL_Keycode)keycodeFromPress:(UIPress *)press
{
    UIKey *key = press.key;
    if (key == nil) {
        return SDLK_UNKNOWN;
    ...
    /* Prefer the printable characters (respects shift/alt state) */
    if (key.characters.length > 0) {
        unichar c = [key.characters characterAtIndex:0];
        switch (c) {
        case '\n':
        case '\r':
            return SDLK_RETURN;
    ...
    switch (key.keyCode) {
    case UIKeyboardHIDUsageKeyboardEscape: return SDLK_ESCAPE;
    case UIKeyboardHIDUsageKeyboardReturnOrEnter: return SDLK_RETURN;
    case UIKeyboardHIDUsageKeyboardTab: return SDLK_TAB;
    case UIKeyboardHIDUsageKeyboardSpacebar: return SDLK_SPACE;
    case UIKeyboardHIDUsageKeyboardDeleteOrBackspace: return SDLK_BACKSPACE;
    case UIKeyboardHIDUsageKeyboardDeleteForward: return SDLK_DELETE;
    case UIKeyboardHIDUsageKeyboardCapsLock: return SDLK_CAPSLOCK;
    case UIKeyboardHIDUsageKeyboardScrollLock: return SDLK_SCROLLLOCK;
    case UIKeyboardHIDUsageKeypadNumLock: return SDLK_NUMLOCKCLEAR;
    case UIKeyboardHIDUsageKeyboardPrintScreen: return SDLK_PRINTSCREEN;
    case UIKeyboardHIDUsageKeyboardPause: return SDLK_PAUSE;
    case UIKeyboardHIDUsageKeyboardInsert: return SDLK_INSERT;
    case UIKeyboardHIDUsageKeyboardHome: return SDLK_HOME;
    case UIKeyboardHIDUsageKeyboardEnd: return SDLK_END;
    case UIKeyboardHIDUsageKeyboardPageUp: return SDLK_PAGEUP;
    case UIKeyboardHIDUsageKeyboardPageDown: return SDLK_PAGEDOWN;
    case UIKeyboardHIDUsageKeyboardUpArrow: return SDLK_UP;
    case UIKeyboardHIDUsageKeyboardDownArrow: return SDLK_DOWN;
    case UIKeyboardHIDUsageKeyboardLeftArrow: return SDLK_LEFT;
    case UIKeyboardHIDUsageKeyboardRightArrow: return SDLK_RIGHT;
    case UIKeyboardHIDUsageKeyboardLeftControl: return SDLK_LCTRL;
    case UIKeyboardHIDUsageKeyboardLeftShift: return SDLK_LSHIFT;
    case UIKeyboardHIDUsageKeyboardLeftAlt: return SDLK_LALT;
    case UIKeyboardHIDUsageKeyboardLeftGUI: return SDLK_LGUI;
    case UIKeyboardHIDUsageKeyboardRightControl: return SDLK_RCTRL;
    case UIKeyboardHIDUsageKeyboardRightShift: return SDLK_RSHIFT;
    case UIKeyboardHIDUsageKeyboardRightAlt: return SDLK_RALT;
    case UIKeyboardHIDUsageKeyboardRightGUI: return SDLK_RGUI;
    default:
        break;
    }
    if (key.keyCode >= UIKeyboardHIDUsageKeyboardF1 && key.keyCode <= UIKeyboardHIDUsageKeyboardF12) {
        return SDLK_F1 + (key.keyCode - UIKeyboardHIDUsageKeyboardF1);
    }
    return SDLK_UNKNOWN;
}

/* Local modification: send any printable text straight to the event system.
   The UIKit text field may not be the first responder in the host launcher
   embedding, so we can't rely on textFieldTextDidChange to deliver
   SDL_EVENT_TEXT_INPUT (0x303) for hardware key presses. */
- (void)sendTextFromPress:(UIPress *)press
{
    UIKey *key = press.key;
    if (key == nil || key.characters.length == 0) {
        return;
    }
    SDL_SendKeyboardText(key.characters.UTF8String);
}
...
- (void)pressesBegan:(NSSet<UIPress *> *)presses withEvent:(UIPressesEvent *)event
{
    if (!SDL_HasKeyboard()) {
        for (UIPress *press in presses) {
            ...
            SDL_Keycode keycode = [self keycodeFromPress:press];
            NSLog(@"[SDLKey] pressesBegan scancode=%d keycode=0x%x chars=%@", scancode, keycode, press.key.characters);
            SDL_SendKeyboardKeyAndKeycode(UIKit_GetEventTimestamp([event timestamp]), SDL_GLOBAL_KEYBOARD_ID, 0, scancode, keycode, true);
            if (SDL_TextInputActive(sdlwindow)) {
                [self sendTextFromPress:press];
            }
        }
    }
    if (SDL_TextInputActive(sdlwindow)) {
...
- (void)pressesEnded:(NSSet<UIPress *> *)presses withEvent:(UIPressesEvent *)event
{
    if (!SDL_HasKeyboard()) {
        for (UIPress *press in presses) {
            SDL_Scancode scancode = [self scancodeFromPress:press];
            SDL_Keycode keycode = [self keycodeFromPress:press];
            NSLog(@"[SDLKey] pressesEnded scancode=%d keycode=0x%x chars=%@", scancode, keycode, press.key.characters);
            SDL_SendKeyboardKeyAndKeycode(UIKit_GetEventTimestamp([event timestamp]), SDL_GLOBAL_KEYBOARD_ID, 0, scancode, keycode, false);
        }
    }
    if (SDL_TextInputActive(sdlwindow)) {
...
- (void)pressesCancelled:(NSSet<UIPress *> *)presses withEvent:(UIPressesEvent *)event
{
    if (!SDL_HasKeyboard()) {
        for (UIPress *press in presses) {
            SDL_Scancode scancode = [self scancodeFromPress:press];
            SDL_Keycode keycode = [self keycodeFromPress:press];
            NSLog(@"[SDLKey] pressesCancelled scancode=%d keycode=0x%x chars=%@", scancode, keycode, press.key.characters);
            SDL_SendKeyboardKeyAndKeycode(UIKit_GetEventTimestamp([event timestamp]), SDL_GLOBAL_KEYBOARD_ID, 0, scancode, keycode, false);
        }
    }
    if (SDL_TextInputActive(sdlwindow)) {
